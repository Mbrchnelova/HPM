...
def TraceStreamlinesBack(max_x_user, nodes, nodes_relevance, us, vs, ws, dt_end,
                         stag_node, epsilon, us_mesh, vs_mesh, ws_mesh, centroids,
                         centroids_ordered, centroids_ordered_dxs, normals,
                         connectivity, xmin, xmax, ymin, ymax, zmin, zmax,
                         le_points, epsilon_LEP, VERIFY, PRINT):
    node_paths_coord = []
    node_paths_elem = []
    nodes_resolved = []
    nodes_resolved_idxs = []
    ...
    for n in range(0, len(nodes)):
        ...
        if abs(local_u_vector[0]) < threshold_vel and abs(local_u_vector[1]) < threshold_vel and abs(local_u_vector[2]) < threshold_vel:
            if PRINT:
                print 'Node in shadow: ', n , ' at ', nodes[n]                                        #THESE ARE ALL FAILURE OF TRACING POSSIBILITIES
            #if VERIFY:
            #    ax.scatter(nodes[n][0], nodes[n][1], nodes[n][2], s=4, c = 'r')
        elif nodes_relevance[n] == 0:
            if PRINT:
                print 'Node irrelevant: ', n , ' at ', nodes[n]                                       #THESE ARE ALL FAILURE OF TRACING POSSIBILITIES
            #if VERIFY:
            #    ax.scatter(nodes[n][0], nodes[n][1], nodes[n][2], s=4, c = 'r')
            ...
            crossed_bool_arr = [crossed]
            for le in range(0, len(le_points)):
                crossed = CrossedStagnationLine(coordinates, le_points[le], epsilon_LEP)[0]
                crossed_bool_arr.append(crossed)

            #print "Crossed bool: ", crossed_bool_arr

            if dist_to_stag_point < epsilon or crossed_bool_arr[0] == True:
            ...
            elif sum(crossed_bool_arr[1:]) >= 1:
                distances = []
                min_dist = 1e9
                min_dist_idx = 0
                for le in range(0, len(le_points)):
                    distance = ((coordinates[0] - le_points[le][0])**2 + (coordinates[1] - le_points[le][1])**2 + (coordinates[2] - le_points[le][2])**2)**0.5
                    distances.append(distance)
                    if distance < min_dist:
                        min_dist_idx = le
                        min_dist = distance

                current_le = le_points[min_dist_idx]
                #path_coord = [[coordinates[0], coordinates[1], coordinates[1]]]
                path_coord = [current_le]
                local_u_vector, element = FindVelVectorNotOrdered(coordinates, us_mesh, vs_mesh, ws_mesh, centroids)
                ...
                crossed_SP = CrossedStagnationLine(coordinates, stag_node, epsilon)[0]
                crossed_bool_arr = []
                crossed_bool_arr.append(crossed_SP)
                for le in range(0, len(le_points)):
                    crossed = CrossedStagnationLine(coordinates, le_points[le], epsilon_LEP)[0]
                    crossed_bool_arr.append(crossed)
                it = 0.
                ...
                distances.append(dist_to_stag_point_previous)
                for le in range(0, len(le_points)):
                    distance = ((coordinates[0] - le_points[le][0])**2 + (coordinates[1] - le_points[le][1])**2 + (coordinates[2] - le_points[le][2])**2)**0.5
                    distances.append(distance)

                while sum(crossed_bool_arr) < 1:

                    if distances[0] > 2.*epsilon:
                        s_to_travel_SP = epsilon
                    else:
                        s_to_travel_SP = epsilon/2.

                    if min(distances) > 2.*epsilon_LEP:
                        s_to_travel_LEP = epsilon_LEP
                    else:
                        s_to_travel_LEP = epsilon_LEP/2.

                    s_to_travel = min(s_to_travel_LEP, s_to_travel_SP)
                    ...
                    if sum(crossed_bool_arr) >= 1 or crossed_bool_arr[0] == True:
                        if crossed_bool_arr[0] == True:
                            ...
                            current_le = le_points[min_dist_idx]
                            dist_to_le_point_current = min_dist
                            #coordinates = path_coord[-1]
                            dist_to_le_point_previous = ((coordinates[0] - current_le[0])**2 + (coordinates[1] - current_le[1])**2 + (coordinates[2] - current_le[2])**2)**0.5
                            dt = dt * dist_to_le_point_previous / (dist_to_le_point_previous + dist_to_le_point_current)
                            ...
                            element_proj = [coordinates[0] - dot_scalar * normal[0], coordinates[1] - dot_scalar * normal[1], coordinates[2] - dot_scalar * normal[2] ]
                            coordinates = element_proj
                            coordinates = current_le

                            where_ended.append(min_dist_idx)
                        ...
                        RESOLVED = True
                        #if VERIFY:
                        #    ax.scatter(path_coord[0][0], path_coord[0][1], path_coord[0][2], s=2, c = 'm')
                        break
                    ...
                    if coordinates[0] < xmin or coordinates[0] > xmax or coordinates[1] < ymin or coordinates[1] > ymax or coordinates[2] < zmin or coordinates[2] > zmax:
                        FAILED = True
                        if PRINT:
                            print 'Out of bounds: ', n , ' at ', nodes[n]
                        break
                    if it > nit_crit:
                        FAILED = True
                        if PRINT:
                            print 'Too many iterations: ', n , ' at ', nodes[n]
                        break
                if FAILED:
                    if PRINT:
    ...
    return intersections, node_paths_elem, node_paths_coord, nodes_resolved, nodes_resolved_idxs, where_ended
