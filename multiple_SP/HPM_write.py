...
#
#
# HPM_write.py: here, all functions responsible for output writing are defined.
# If modified, the respective reading functions should also be adjusted accordingly in HPM_read.  
#
...
def WriteLEPointData(filename, stag_points, epsilon):
    f = open(filename, "w")

    i = 0
    for stag_point in stag_points:
        line = str(stag_point[0]) + ' ' +  str(stag_point[1]) + ' ' +  str(stag_point[2]) + ' ' + str(epsilon) + '\n'
        f.write(line)
        i += 1
    f.close()
...
def WriteLEPThermoData(filename, LEPs_all, epsilon_LEP, Thermo_all):
    f = open(filename, "w")

    i = 0
    for l in range(0, len(LEPs_all)):
        u_LEP = Thermo_all[l][0]
        v_LEP = Thermo_all[l][1]
        w_LEP = Thermo_all[l][2]
        T_LEP = Thermo_all[l][3]
        mu_LEP = Thermo_all[l][4]
        rho_LEP = Thermo_all[l][5]
        p_LEP = Thermo_all[l][6]
        M_LEP = Thermo_all[l][7]

        line = str(u_LEP) + ' ' + str(v_LEP) + ' ' + str(w_LEP) + ' ' + str(T_LEP) + ' ' + str(mu_LEP) + ' ' + str(rho_LEP) + ' ' + str(p_LEP) + ' ' + str(M_LEP) + '\n'
        f.write(line)
        i += 1
    f.close()
...
def WriteBackTracing(filename_intersections, filename_nodepaths, filename_nodecoords, filename_nodesresolved, filename_epsilonnodes, filename_whereended, intersections, node_paths_elem, node_paths_coord, epsilon_nodes, nodes_resolved_idxs, where_ended):
    ...
    f = open(filename_whereended, "w")
    for n in range(0, len(nodes_resolved_idxs)):
        where_ended_n = where_ended[n]
        line = ""
        line += str(where_ended_n) + '\n'
