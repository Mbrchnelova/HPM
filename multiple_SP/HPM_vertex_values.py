...
def GetVertexValuesFaster(nodes, centroids, connectivity, u_centroids, v_centroids, w_centroids, p_centroids):
    ...
    for i in range(0, len(nodes)):
        ...
        if how_many_at_i == 0:
            print "Not all nodes included in the connectivity matrix. Either fix your mesh or use IDS for value interpolations. Node:", nodes[i], "is a problem. Will be interpolated."
            alone.append(i)
    ...
    for i in range(0, len(alone)):
        idx = alone[i]
        u_nodes[idx] = u_nodes[idx+1]
        v_nodes[idx] = v_nodes[idx+1]
        w_nodes[idx] = w_nodes[idx+1]
        p_nodes[idx] = p_nodes[idx+1]
