...
def MarchDownstream(stg_coord, stg_cnds, wdydbetaz_ini, list_of_crossed_elements,
                    list_of_coordinates, h, GradFs, Fxs, us_ele, vs_ele, ws_ele,
                    T_ele, mu_ele, rho_ele, M_ele, w_fit, T_w, cp, k, wds,
                    metric_coeffs, INTERPOLATE_N, OWN_TRANSITION, PERZHIKAR, SP):
    ...
    for i in range(1, len(list_of_crossed_elements)):
        ...
        if h <= 0.:
            print "ERROR IN METRIC DATA", list_of_coordinates[-1]
            break
        V_e = V
        T_e = T_ele[el]
        M_e = M_ele[el]
        ...
        dotq_w, delta, theta, Re_theta_e, integ_tracing, skin_cf = SolveThermal(transitioned,
                                            ...
                                            rho_star_old, V_e_old, u_e_old, h_old, rho_e_old, integ_tracing, FIRST, INTERPOLATE_N, x_coord)
        ...
        if transitioned and NOTIFY_TRANSITION:
            #print "Transition occurred at eq. body radius of:", h, " and xcoord of ", x_coord,  " with Re_T of: ", Re_theta_e, Pr
        ...
        FIRST = False
    return hs, all_points, thetas, dotq_ws, deltas, transition_points, V_es, rho_es, mu_stars, rho_stars, integs, dss, skin_cfs, Re_theta_es, M_es, T_es
