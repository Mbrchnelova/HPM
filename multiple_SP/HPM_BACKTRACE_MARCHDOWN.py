...
if VISCOUS:
    ...
    le_points, epss_LEP = ReadLePointData(filename_le_point)
    le_thermal = ReadLeThermalData(filename_le_thermal)
    ...
    if WRITE_BACKTRACING:
        WRITTEN = WriteBackTracing(filename_intersections, filename_nodepaths, filename_nodecoords, filename_nodesresolved, filename_epsilonnodes, filename_whereended, intersections, node_paths_elem, node_paths_coord, epsilon_nodes, nodes_resolved_idxs, where_ended)

    else:
        print "Reading intersection data..."
        intersections, node_paths_elem, node_paths_coord, epsilon_nodes, nodes_resolved_idxs, where_ended = ReadBacktracingData(filename_intersections, filename_nodepaths, filename_nodecoords, filename_nodesresolved, filename_epsilonnodes, filename_whereended)
    ...
    final_values_Rethetaes = []
    final_values_Mes = []
    final_values_Tes = []

    print "Computing SP heat flux..."
    Tstag_behindNSW, rhostag_behindNSW, pstag_behindNSW, mustag_behindNSW, Vstag_behindNSW = GetStagPointConditions(Mach, pinf, rhoinf, Tinf, R)
    ...
    if PERZHIKAR and MARCH:
        ...
        metriccoeffs = ComputeMetricPerzhikar(nodes, nodes_resolved_idxs, node_paths_coord, stag_point, dist_yz_min, where_ended)
    ...
    if MARCH:
        ...
        for s in range(0, len(epsilon_nodes)):
            ...
            if where_ended[s] == 'SP':
                hs_res, hs_coords, thetas, dotq_ws, deltas, transition_points, V_es, rho_es, mu_stars, rho_stars, integs, dss, skin_cfs, Rethetaes, Mes, Tes = MarchDownstream(stag_point, stg_cnds, wdydbetaz_ini, list_of_crossed_elements, list_of_coordinates, h_ini, GradFs, Fxs, u_elements, v_elements, w_elements, T_elements, mu_elements, rho_elements, M_elements, w_fitted, T_w, cp, k, wds, metric_coeffs_current, INTERPOLATE_N, OWN_TRANSITION, PERZHIKAR, SP=True)

            else:
                ended_at = int(where_ended[s])
                LE_th = le_thermal[ended_at]
                le_point = le_points[ended_at]
                u_LEP = LE_th[0]
                v_LEP = LE_th[1]
                w_LEP = LE_th[2]
                T_LEP = LE_th[3]
                ...
                p_LEP = LE_th[6]
                M_LEP = LE_th[7]
                V_LEP = (u_LEP**2 + v_LEP**2 + w_LEP**2)**0.5

                #le_cnds = [Tstag_behindNSW, rhostag_behindNSW, pstag_behindNSW, mustag_behindNSW, Vstag_behindNSW]
                le_cnds = [T_LEP, rho_LEP, p_LEP, mu_LEP, V_LEP, M_LEP, u_LEP]
                hs_res, hs_coords, thetas, dotq_ws, deltas, transition_points, V_es, rho_es, mu_stars, rho_stars, integs, dss, skin_cfs, Rethetaes, Mes, Tes = MarchDownstream(le_point, le_cnds, wdydbetaz_ini, list_of_crossed_elements, list_of_coordinates, h_ini, GradFs, Fxs, u_elements, v_elements, w_elements, T_elements, mu_elements, rho_elements, M_elements, w_fitted, T_w, cp, k, wds, metric_coeffs_current, INTERPOLATE_N, OWN_TRANSITION, PERZHIKAR, SP=False)
            ...
            if len(hs_coords) > 0.:
                ...
                if hs_coords[-1][0] > -1.:
                    ...
                    final_values_Rethetaes.append(Rethetaes[-1])
                    final_values_Mes.append(Mes[-1])
                    final_values_Tes.append(Tes[-1])
                    ...
                    final_values_Rethetaes.append(Rethetaes[0])
                    final_values_Mes.append(Mes[0])
                    final_values_Tes.append(Tes[0])



    print 'Viscous Solver marchdown stop time:', time.time()

    final_values_Tes.append(Tstag_behindNSW)
